...
export const generationPrompts = {
  npc: 'Create one campaign NPC with a stable personal canonical name. Keep the player-facing unknown-person descriptor in display_label, the ordinary job in profession, and rank or office in title. Never use an appearance phrase, job, numbered role, or unidentified label as the canonical name. class_levels are actual rules-bearing classes only, never a synonym for profession. Set narrative_depth only as far as requested: surface for incidental presence, developed for meaningful interaction, major only for recurring story-bearing people. Set mechanical_depth independently. Include mechanical_title_basis only when the title has explicit class, hit-dice, feature, or feat meaning. Reconcile any supplied canonical actor before generating and preserve locked facts. Generate combat mechanics only when mechanical_depth requires them.',
