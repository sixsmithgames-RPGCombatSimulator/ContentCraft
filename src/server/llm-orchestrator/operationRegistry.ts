...
const storyDirectorSceneDesign = {
  ...
  properties: {
    schemaVersion: { enum: ['gmc.scene-story-design/1', 'gmc.scene-story-design/2'] },
    ...
    storyFactBindings: {
      type: 'array', maxItems: 24, items: {
        type: 'object', additionalProperties: false,
        required: ['bindingId', 'instructionFingerprint', 'semanticNodeRef', 'requirementRef', 'requirementFingerprint', 'affordanceRef', 'factRefs', 'resolutionMode'],
        properties: {
          bindingId: { type: 'string', minLength: 1, maxLength: 240 },
          instructionFingerprint: { type: 'string', pattern: '^[a-f0-9]{64}$' },
          semanticNodeRef: { type: 'string', minLength: 1, maxLength: 240 },
          requirementRef: { type: 'string', minLength: 1, maxLength: 240 },
          requirementFingerprint: { type: 'string', pattern: '^[a-f0-9]{64}$' },
          affordanceRef: { type: 'string', minLength: 1, maxLength: 240 },
          factRefs: { type: 'array', minItems: 1, maxItems: 8, uniqueItems: true, items: { type: 'string', minLength: 1, maxLength: 240 } },
          resolutionMode: { enum: ['direct', 'provisional_check'] },
        },
      },
    },
...
const actionDirectedStoryTurnOutput = {
  schemaVersion: { enum: ['gma.story-director-result/2', 'gma.story-director-result/3', 'gma.story-director-result/4', 'gma.story-director-result/5'] },
...
const actionDirectedCurrentSceneOutput = {
  schemaVersion: { enum: ['gma.current-scene-narration-result/4', 'gma.current-scene-narration-result/5', 'gma.current-scene-narration-result/6', 'gma.current-scene-narration-result/7', 'gma.current-scene-narration-result/8', 'gma.current-scene-narration-result/9', 'gma.current-scene-narration-result/10', 'gma.current-scene-narration-result/11'] },
  programId: { type: 'string', minLength: 1, maxLength: 240 },
  nodeId: { type: 'string', minLength: 1, maxLength: 240 },
...
const seeds: Seed[] = [
  { id: 'intent.classify', operationClass: 'structured_low', tier: 'structured', required: ['intentType', 'confidence', 'structuredIntent', 'requiresVcs', 'requiresGameMasterCraft'], optional: ['actionPlan', 'ambiguities', 'dataRequirements'], targetBytes: 8_000, hardLimitBytes: 16_000, maxOutputTokens: 700, thinkingLevel: 'minimal', maxAttempts: 1, fallbackAllowed: false },
  {
    ...
    promptVersion: 'gma.semantic-intent-policy/19',
    outputProperties: {
    ...
    systemInstruction: [
      ...
      'Give every retained typed information outcome one unique targetId + facet + relationOriginTargetId address. Distinct requested subjects or phenomena use distinct local subject targets even when they share an enclosing place, actor, container, evidence, observer, method, viewpoint, or group; do not bind them all to the enclosing target. Copy authorityRef only for the precise subject. Reuse a non-null authorityRef only when case/whitespace-normalized target descriptions name the same subject. Different derived subjects (purpose, route, risk, pattern, relation) use null unless each has a precise catalog ref. Consolidate redundant wording instead of duplicate addresses.',
  ...
  },
  {
    ...
    systemInstruction: [
      ...
      'Set proposal.preparationProfile.minimumDepth, and its exact sceneReality copy, no shallower than buildRequest.deterministicMinimumDepth and no deeper than the selected requestedDepth. A semantic depth judgment may raise this effective floor; never lower it back to the deterministic floor when the prepared Scene requires the deeper depth.',
  ...
  },
  {
    ...
    systemInstruction: [
      ...
      'Return exactly one JSON object matching the requested gma.story-director-result version, including gma.story-director-result/5 for current packets. This is proposal-only; GMC remains Story and canon authority and VCS remains mechanics authority.',
      ...
      'When the supplied actionBoundReveal reports blocking scene-substance debt, replace the same Scene kit in place and add the minimum concrete information required by the structured semantic outcome. For every story_fact outcome, authorize that information only through its exact storyFactBindings identity and reference closure; labels, verbs, nouns, synonyms, and prose overlap are not authority. State every direct bound fact in openingNarration, or use provisional_check with one legal VCS check and five finished branches.',
  ...
  },
  {
    ...
    thinkingLevel: 'medium', maxAttempts: 1, fallbackAllowed: false, promptVersion: 'gma.current-scene-narration-policy/17',
    ...
    systemInstruction: [
      ...
      'Return exactly one JSON object using the current-scene result schemaVersion requested by the bounded GMA packet: gma.current-scene-narration-result/9 for fresh settled typed-observation narration, gma.current-scene-narration-result/11 for fresh Story current-scene narration, or /10, /8, /7, /6, /5, or /4 only for an older compatible packet during ordered rollout. Do not propose, replace, move, or close the Scene kit.',
      'For gma.current-scene-narration-result/9, return exactly these top-level fields and no others: schemaVersion, programId, nodeId, presentationFingerprint, responseText, presentationBindings, materialClaims, rulesNote. Copy programId, nodeId, and presentationFingerprint exactly. Compose and silently edit lived prose first, faithfully realize every permitted outcome, then bind each outcome to an exact excerpt from the final responseText and return exactly one matching material claim with unchanged source refs. Ordinary descriptive statements may be paraphrased naturally; preserve every listed locked value, including exact names, quantities, measurements, quoted speech, and mechanics. Historical /8 keeps its original verbatim permitted-statement contract and the same observation-only top-level field set.',
      'Fresh /9 and historical /8 must omit these non-observation sibling fields: proposedTimeAdvance, responseMode, rollRequest, sceneRealization, declaredActionPayoff, storyOutcome, agencyAudit, mechanicsAuthority.',
      ...
      'For non-observation current-Scene result families only, obey the supplied temporalRequirement in the first result. Required means proposedTimeAdvance has shouldAdvance true, positive whole seconds no greater than seven days, the exact required activity, and a concrete reason; forbidden means null. This is only a proposal for GMC campaign-time authority. Fresh /9 and historical /8 omit proposedTimeAdvance.',
      ...
      'Use supplied participant context for purposeful NPC behavior, dialogue, objectives, and immediate decisions without exposing private motives as player-known fact. Use the GMC scene-time context as read-only authority; pending events remain possibilities until play or elapsed time makes their effects observable.',
      ...
      'For non-observation current-Scene result families with typed story-fact bindings, direct resolution must pay off every mapped requirement in current responseText without a check. provisional_check resolution requires one legal pending VCS skill check and all five finished branches. Never replace or widen the supplied binding by comparing actor labels, requirement wording, access prose, fact wording, synonyms, or token overlap.',
      ...
      'For non-observation current-Scene result families, a provisional check on a story-bearing target returns all five prepared branches with gma.substantive-outcome/2. Each branch must state a concrete fact-bound finding, scope-limited negative, or specific barrier; process-only prose such as “the load is established” is not a result.',
      'For non-observation current-Scene result families, when storyAffordanceProjection is supplied, any claimed actualStoryImpact must reference a projected obligation, use an allowed contribution, and include gma.story-satisfaction-receipt/1. State the receipt’s concrete answer, confirmation, complication, consequence, or decision explicitly in responseText; metadata alone is never a result. Cite only supplied factRefs, use an effect-compatible obligationState, leave remainingQuestion empty only when resolved, and return no impact or receipt when no obligation changed.',
      'For non-observation current-Scene result families, return gma.scene-realization/1. Cover every requested responder or cohort member with exact prose evidence; describe a cohort collectively with all, each, both, or its known count, or use one observedCount 1 row with different evidence per member; account for requested concealment and every action-matched capability. Fresh /9 and historical /8 express supported behavior directly in responseText and omit sceneRealization.',
      'Keep rules analysis out of responseText. Non-observation result families put the lived effect or non-effect in prose and an unsupported capability rule only in mechanicsNote. Fresh /9 and historical /8 keep rulesNote null.',
      'Make exposure or loss conditional on a failed, detected, conspicuous, delayed, or otherwise concrete risky course. Merely taking another action cannot fail the scene.',
