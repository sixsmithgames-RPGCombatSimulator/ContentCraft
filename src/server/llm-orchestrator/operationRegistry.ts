  ...
  { id: 'intent.classify', operationClass: 'structured_low', tier: 'structured', required: ['intentType', 'confidence', 'structuredIntent', 'requiresVcs', 'requiresGameMasterCraft'], optional: ['actionPlan', 'ambiguities', 'dataRequirements'], targetBytes: 8_000, hardLimitBytes: 16_000, maxOutputTokens: 700, thinkingLevel: 'minimal', maxAttempts: 1, fallbackAllowed: false },
  {
    ...
    outputProperties: {
      ...
      intents: {
        ...
        items: {
          type: 'object', additionalProperties: false,
          required: ['intentId', 'summary', 'evidenceQuotes', 'goal', 'purpose', 'targets', 'observerTargetId', 'methods', 'requestedOutcomes', 'relation'],
          properties: {
            ...
            targets: { type: 'array', maxItems: 8, items: { type: 'object', additionalProperties: false, required: ['targetId', 'role', 'description', 'authorityRef'], properties: { targetId: { type: 'string', minLength: 1, maxLength: 240 }, role: { enum: ['actor', 'recipient', 'subject', 'object', 'origin', 'destination', 'area'] }, description: { type: 'string', minLength: 1, maxLength: 500 }, authorityRef: { anyOf: [{ type: 'null' }, { type: 'string', minLength: 1, maxLength: 240 }] } } } },
            observerTargetId: { anyOf: [{ type: 'null' }, { type: 'string', minLength: 1, maxLength: 240 }] },
            methods: { type: 'array', maxItems: 8, items: { type: 'object', additionalProperties: false, required: ['methodId', 'kind', 'description', 'capabilityHint', 'authorityRef'], properties: { methodId: { type: 'string', minLength: 1, maxLength: 240 }, kind: { enum: ['approach', 'capability', 'spell', 'item', 'tool', 'other'] }, description: { type: 'string', minLength: 1, maxLength: 500 }, capabilityHint: { anyOf: [{ type: 'null' }, { type: 'string', minLength: 1, maxLength: 160 }] }, authorityRef: { anyOf: [{ type: 'null' }, { type: 'string', minLength: 1, maxLength: 240 }] } } } },
            requestedOutcomes: { type: 'array', minItems: 1, maxItems: 8, items: { anyOf: [
              { type: 'string', minLength: 1, maxLength: 500 },
              { type: 'object', additionalProperties: false, required: ['outcomeId', 'targetRef', 'facet', 'valueKind', 'requestedPrecision', 'evidenceQuotes'], properties: {
                ...
                requestedPrecision: { enum: ['ordinary', 'bounded', 'exact'] },
                evidenceQuotes: { type: 'array', minItems: 1, maxItems: 8, items: { type: 'string', minLength: 1, maxLength: 1_000 } },
              } },
            ] } },
    ...
    systemInstruction: [
      'Interpret one exact player instruction into one bounded gma.semantic-intent-ir/2 JSON object. Return only that object.',
      ...
      'For discover_information or observe_situation, emit one typed requestedOutcomes object per separately requested answer. Use surface_description for visible appearance, apparent_classification for apparent ancestry, species, or type, identity only for established identity, spatial_relation for distance or relative position, contents for what an area or container holds, activity for what a subject is doing, presence for whether something is there, quantity for how many, extent for size or reach, condition for observable state, and signal for a signal or response. Never collapse these facets into one generic observation.',
      'Every observation outcome targetRef must copy an exact ref supplied by the request reference catalog through a target authorityRef. If no catalog ref identifies the subject, leave authorityRef null and report one focused ambiguity. Never invent an authority ref, join by a label, or answer the observation yourself.',
      'Set observerTargetId to the targetId of the creature or sensor through which the player observes, including a familiar. Set it null only when the observing actor is not separately declared. Give every target and method a stable local ID. A method may copy an exact authorityRef from the supplied catalog; otherwise leave it null. These fields describe the request and never establish what the observer perceives.',
      'Use description, classification, identity_ref, measurement, measurement_range, relation, boolean, count, set, or statement for a concrete value kind. Use measurement_or_relation only when ordinary language asks how far or where without requiring exact precision.',
