...
gmcV1Router.post('/ai/generate-narration', asyncRoute((req, res) => ai(req, res, 'Continue directly from the most recent exchange in conversationHistory using the supplied authoritative VCS result and campaign memoryContext. Treat earlier narration as established scene state unless a later retcon supersedes it. Never restart, replay, or duplicate an already narrated action. Never invent, change, or recompute mechanical numbers. Do not contradict locked facts. Stop at the next player decision. Canon proposals must use recordType FACT, ITEM, or EVENT; FACT includes scope, ITEM includes itemTier/location/ownership, and EVENT includes a deadline plus the consequence of inaction. Return {narration, npcDialogue, requiresVcsResolution, proposedCanonChanges, proposedVcsExports, riskLevel, syncNotes, gmPrivateNotes}.', ['narration', 'proposedCanonChanges', 'proposedVcsExports', 'riskLevel', 'syncNotes'])));
