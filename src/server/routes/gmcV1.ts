...
Apply every envelope rule. Narrate only the newly authorized action. Preserve unaffected continuity and never invent mechanics, player actions, inventory changes, elapsed time, or canon. Treat canonEvidence.scenePresence as the exact starting roster. If interactionResolution grounds an arrival or departure postcondition, return the resulting roster in sceneSegmentUpdate; GMA will obtain and commit GMC's exact projected presence contract before the narration can apply.
For any item that leaves or enters the player character's inventory, use interactionResolution authority vcs_sheet and return an exact proposedSheetMutation using the supplied VCS item names and quantities. Never use scene_custody to bypass a character-sheet ownership change. Narrate the completed transfer only when the structured mutation is present; do not expose sheet, validation, authority, or transaction diagnostics in player-facing prose.
...
Return exactly one JSON object matching responseContract. For narration return {narration,interactionResolution,proposedSheetMutation,npcDialogue,requiresVcsResolution,proposedCanonChanges,proposedVcsExports,proposedTimeAdvance,sceneSegmentUpdate,riskLevel,syncNotes,gmPrivateNotes}.`;
...
For player_roll, also return outcomeProse with criticalFailure, partial, success, greatSuccess, and criticalSuccess. Each branch is 45-140 words of finished, vivid, scene-specific player-facing narration for that exact result: concrete motion, sensory evidence, discovered information, cost, complication, or earned advantage. Never return an abstract placeholder such as “the approach works,” “gets the core information,” or “the result determines whether.” The branches are provisional and may not claim that any one happened before VCS resolves the roll.
Return {resolutionMode,skill,ability,dc,rollMode,passiveEligible,reason,preRollNarration,stakes,outcomeProse,estimatedTimeCost,confidence}, plus requiredChecks only when distinct checks are all necessary. For no_roll, passive, or hidden_roll, outcomeProse may be omitted.`;
...
gmcV1Router.post('/ai/generate-narration', asyncRoute((req, res) => ai(
  ...
  ['narration', 'interactionResolution', 'proposedCanonChanges', 'proposedVcsExports', 'riskLevel', 'syncNotes'],
)));
...
Treat the supplied scenePresenceContract as the exact roster authorized for this candidate. It may be GMC's current starting contract or a revision-bound projected contract already resolved from interactionResolution. Mark the candidate invalid when an NPC outside that supplied contract acts, speaks, observes, carries evidence, guards, or receives an assignment. Do not silently change the roster inside prose; report that the structured interaction/scene transition must be repaired so GMA can resolve it before narration.
