...
gmcV1Router.post('/ai/generate-narration', asyncRoute((req, res) => ai(req, res, 'Continue directly from the most recent exchange in conversationHistory using the supplied authoritative VCS result and campaign memoryContext. Treat earlier narration as established scene state unless a later retcon supersedes it. Never restart, replay, or duplicate an already narrated action. Never invent, change, or recompute mechanical numbers. Do not contradict locked facts. The supplied dmCheckPolicy is binding: use playerCharacter capabilities and passive scores, never surface hidden reads, call a check only when every eligibility test passes, and describe the DC plus every scene-specific outcome band before requesting a roll. Stop at the next player decision. Canon proposals must use recordType FACT, ITEM, or EVENT; FACT includes scope, ITEM includes itemTier/location/ownership, and EVENT includes a deadline plus the consequence of inaction. Return {narration, npcDialogue, requiresVcsResolution, proposedCanonChanges, proposedVcsExports, riskLevel, syncNotes, gmPrivateNotes}.', ['narration', 'proposedCanonChanges', 'proposedVcsExports', 'riskLevel', 'syncNotes'])));
gmcV1Router.post('/ai/respond-ooc', asyncRoute((req, res) => ai(req, res, 'Respond out of character as the DM to the player, using conversationHistory and campaign context. Do not narrate the player character taking actions. Clearly distinguish rules/table discussion from in-world facts. If the player is flagging a continuity problem, explain what you understand and what would change. Return {response,continuityNotes,proposedCanonChanges}.', ['response', 'continuityNotes', 'proposedCanonChanges'])));
gmcV1Router.post('/ai/retcon-narration', asyncRoute((req, res) => ai(req, res, 'Apply the player retcon instruction as an authoritative correction to the recent conversationHistory. Discard contradicted narration, preserve everything not affected, and continue from the corrected current state without replaying earlier beats. Do not alter VCS mechanics unless an authoritative restored VCS snapshot is supplied. If the corrected continuation reaches a check, the supplied dmCheckPolicy is binding and all stakes must be disclosed before requesting the roll. Return {narration,correctionSummary,proposedCanonChanges,continuityNotes}.', ['narration', 'correctionSummary', 'proposedCanonChanges', 'continuityNotes'])));
...
gmcV1Router.post('/ai/extract-canon-changes', asyncRoute((req, res) => ai(req, res, `Extract only durable story memory from the transcript. Do not turn raw mechanics into canon unless story-relevant.
Classify each proposal as exactly one recordType:
- FACT: a truth, with scope {kind:'geographic',tier:'world|city|district|site|room',locationId} or {kind:'entity',tier:'bbeg|lieutenant|henchman|contact',entityId}.
- ITEM: a discrete physical object, with itemTier 'plot|mundane|currency|furniture', currentLocationId, and ownerEntityId/ownerType when known.
- EVENT: a pending pressure that will occur unless interrupted, with deadlineAt or deadlineDescription, consequence, and scope.
Return {proposedCanonChanges:[{recordType,changeType,targetId,summary,payload,scope,itemTier,deadlineAt,deadlineDescription,consequence,riskLevel,requiresHardApproval,reason}]}.`, ['proposedCanonChanges'])));
