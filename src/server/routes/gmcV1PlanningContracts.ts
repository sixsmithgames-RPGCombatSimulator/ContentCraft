...
- campaignDashboard.scenePresenceContract is the exact starting roster. A known non-present NPC cannot act in that starting state. When the supplied interaction contract grounds an arrival or departure postcondition, declare it structurally in interactionResolution and the resulting scene roster; GMA must obtain and commit GMC's revision-bound projected presence contract before any narration using that NPC applies.
