...
// Server-safe product key resolver
export function getProductKey(request?: any): ProductKey {
  // Check environment variable first (primary source for server-side)
  const configured = process.env.NEXT_PUBLIC_PRODUCT_KEY || "";
  ...
  // Check query parameter if request is available
  if (request && typeof request.url === "string") {
    try {
      ...
      // Invalid URL, continue to hostname detection
    }
  }

  // Check hostname from headers if request is available
  if (request && typeof request.headers?.get === "function") {
    const host = request.headers.get("host");
    if (host) {
      const hostname = host.toLowerCase();
      if (hostname.includes("gamemastercraft") || hostname.includes("gmcraft")) {
        return "gamemastercraft";
      }
      if (hostname.includes("sagacraft")) {
        return "sagacraft";
      }
    }
  }
...
export function getProductConfig(request?: any): ProductConfig {
  return PRODUCT_CONFIGS[getProductKey(request)];
}
