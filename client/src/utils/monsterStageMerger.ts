/**
 * Monster Stage Merger - Intelligent merging of multi-stage monster generation
 *
 * Handles conflicts, tracks provenance, and merges data according to monster schema
 *
...
export interface MonsterMergeResult {
  merged: JsonRecord;
...
/**
 * Merge monster stages with conflict detection
 */
export function mergeMonsterStages(stageResults: Record<string, JsonRecord>): MonsterMergeResult {
  ...
  console.log('[Monster Stage Merger] Available stage keys:', Object.keys(stageResults));
  ...
  console.log('[Monster Stage Merger] Merge complete:', {
 ...
 */
export function formatMonsterConflictsForReview(conflicts: FieldConflict[]): string {
  if (conflicts.length === 0) {
    return 'No conflicts detected during monster stage merging.';
  }

  const lines = ['**Monster Stage Merge Conflicts:**\n'];
