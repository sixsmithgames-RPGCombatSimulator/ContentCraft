/**
 * Encounter Stage Merger - Intelligent merging of multi-stage encounter generation
 *
 * Handles conflicts, tracks provenance, and merges data according to encounter schema
 *
...
export interface EncounterMergeResult {
  merged: JsonRecord;
...
function mergeValues(field: string, oldValue: unknown, newValue: unknown): {
  ...
  if (isArray(oldValue) && isArray(newValue)) {
    // Deduplicate proposals by question text
    if (field === 'proposals') {
    ...
    // For arrays of objects (monsters, npcs, hazards, etc.), prefer new but keep old if different names
  ...
  // For non-arrays/objects, last stage wins
...
/**
 * Merge encounter stages with conflict detection
 */
export function mergeEncounterStages(stageResults: Record<string, JsonRecord>): EncounterMergeResult {
  ...
  console.log('[Encounter Stage Merger] Available stage keys:', Object.keys(stageResults));
  ...
  console.log('[Encounter Stage Merger] Merge complete:', {
 ...
 */
export function formatEncounterConflictsForReview(conflicts: FieldConflict[]): string {
  if (conflicts.length === 0) {
    return 'No conflicts detected during encounter stage merging.';
  }

  const lines = ['**Encounter Stage Merge Conflicts:**\n'];
